...
for k = 2:2
    ...
    lst_slstr_n = imresize(lst_slstr_n,0.5,'bilinear');
    
    mydata = Tiff(infile2,'r+');
    mydata.setDirectory(1);
    lst_slstr_o = mydata.read();
    lst_slstr_o = imresize(lst_slstr_o,0.5,'bilinear');
...
%     lst_ahi = imresize(lst_ahi,2);
    ...
    red_slstr_n = mydata.read();
     red_slstr_n = imresize(red_slstr_n,0.5,'bilinear');
        
    mydata = Tiff(infile5,'r+');
    mydata.setDirectory(1);
    red_slstr_o = mydata.read();
     red_slstr_o = imresize(red_slstr_o,0.5,'bilinear');
...
%         y_ = [y_;y];
    ...
    outfile = [outdir 'Aus_201801' num2str(k,'%02d') '_3VTa.tif'];
    saveTif(coeffa,outfile);
    outfile = [outdir 'Aus_201801' num2str(k,'%02d') '_3VTb.tif'];
    saveTif(coeffb,outfile);
    outfile = [outdir 'Aus_201801' num2str(k,'%02d') '_3VTc.tif'];
...
%     text(35,12,{['RMSE =' num2str(rmse,'%.2f') '¡ãC' ] ['Bias = ' num2str(bias,'%.2f') '¡ãC'] 
%         ['R^2 = ' num2str(r2,'%.2f')]  ['\sigma = ' num2str(sigma,'%.2f') '¡ãC']},...
