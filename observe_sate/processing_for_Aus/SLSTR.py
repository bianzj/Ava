...
class SLSTR:
    ...
    def inversionAOD_nadir(self, wdir, dataTime):
        ...
        infile = wdir + r'heihe\heihe_' + dataTime + '_refl_red_n.tif'
        ...
        infile = wdir + r'heihe\heihe_' + dataTime + '_vza_n.tif'
        [vza, temp1, temp2, temp3, temp4, temp5] = self.getTiffData(infile)
        infile = wdir + r'heihe\heihe_' + dataTime + '_psi_n.tif'
        ...
        outfile = wdir + r'heihe_result\heihe_' + dataTime + '_aod_n.tif'
    ...
    def inversionAOD_obliq(self, wdir, dataTime):
        ...
        # infile = wdir+r'heihe_'+dataTime+'_refl_nir.tif'
        # [refl_nir,temp1,temp2,temp3,temp4] = self.getTiffData(infile)
        infile = wdir + r'heihe\heihe_' + dataTime + '_refl_ref_o.tif'
        [refl_ref, temp1, temp2, temp3, temp4, temp5] = self.getTiffData(infile)
        # refl_ref = self.resize(refl_ref, nl, ns)
        infile = wdir + r'heihe\heihe_' + dataTime + '_sza_o.tif'
        [sza, ns, nl, nb, geog, proj] = self.getTiffData(infile)
        rd = np.pi / 180.0
        refl_red = refl_red / np.cos(sza * rd)
        refl_ref = refl_ref / np.cos(sza * rd)
        infile = wdir + r'heihe\heihe_' + dataTime + '_vza_o.tif'
        [vza, temp1, temp2, temp3, temp4, temp5] = self.getTiffData(infile)
        infile = wdir + r'heihe\heihe_' + dataTime + '_psi_o.tif'
        [psi, temp1, temp2, temp3, temp4, temp5] = self.getTiffData(infile)
        ...
        outfile = wdir + r'heihe_result\heihe_' + dataTime + '_aod_o.tif'
        self.writeTiff(aod, ns, nl, 1, geog, proj, outfile)
    ...
    def VNIRatm_nadir(self, wdir, dataTime):
        ...
        infile = wdir + r'heihe\heihe_' + dataTime + '_refl_red_n.tif'
        [refl_red, ns, nl, nb, geog, proj] = self.getTiffData(infile)
        infile = wdir + r'heihe\heihe_' + dataTime + '_refl_nir_n.tif'
        [refl_nir, ns, nl, nb, geog, proj] = self.getTiffData(infile)
        # ns = np.int(ns*0.5)
        # nl = np.int(nl*0.5)
        # refl_red = self.resize(refl_red,nl,ns)
        infile = wdir + r'heihe\heihe_' + dataTime + '_rad_red_n.tif'
        [rad_red, temp1, temp2, temp3, temp4, temp5] = self.getTiffData(infile)
        # rad_red = self.resize(rad_red, nl, ns)
        infile = wdir + r'heihe\heihe_' + dataTime + '_rad_nir_n.tif'
        ...
        if (ccc == 0):
            return 0
        # infile = wdir+r'heihe\heihe_'+dataTime+'_refl_nir.tif'
        # [refl_nir,temp1,temp2,temp3,temp4,temp5] = self.getTiffData(infile)
        # infile = wdir+r'heihe\heihe_'+dataTime+'_refl_ref.tif'
        # [refl_ref,temp1,temp2,temp3,temp4,temp5] = self.getTiffData(infile)
        # infile = wdir + r'heihe\heihe_' + dataTime + '_sza_n.tif'
        ...
        ndvi[ndvi < 0] = 0
        ndvi[ndvi > 1] = 1
        outfile = wdir + r'heihe_result\heihe_' + dataTime + '_ndvi_n.tif'
        self.writeTiff(ndvi, ns, nl, 1, geog, proj, outfile)
        # outfile = wdir+r'heihe_result\heihe_'+dataTime+'_red_n.tif'
        # self.writeTiff(reflnewred, ns, nl, 1, geog, proj, outfile)
        # outfile = wdir+r'heihe_result\heihe_'+dataTime+'_nir_n.tif'
        # self.writeTiff(reflnewnir, ns, nl, 1, geog, proj, outfile)

        return 1

    def VNIRatm_obliq(self, wdir, dataTime):
        ...
        infile = wdir + r'heihe\heihe_' + dataTime + '_refl_red_o.tif'
        [refl_red, ns, nl, nb, geog, proj] = self.getTiffData(infile)

        infile = wdir + r'heihe\heihe_' + dataTime + '_refl_nir_o.tif'
        [refl_nir, ns, nl, nb, geog, proj] = self.getTiffData(infile)
        # ns = np.int(ns*0.5)
        # nl = np.int(nl*0.5)
        # refl_red = self.resize(refl_red,nl,ns)
        infile = wdir + r'heihe\heihe_' + dataTime + '_rad_red_o.tif'
        [rad_red, temp1, temp2, temp3, temp4, temp5] = self.getTiffData(infile)
        # rad_red = self.resize(rad_red, nl, ns)
        infile = wdir + r'heihe\heihe_' + dataTime + '_rad_nir_o.tif'
        [rad_nir, temp1, temp2, temp3, temp4, temp5] = self.getTiffData(infile)
        # rad_nir = self.resize(rad_nir, nl, ns)
        ...
        ndvi[ndvi < 0] = 0
        ndvi[ndvi > 1] = 1
        outfile = wdir + r'heihe_result\heihe_' + dataTime + '_ndvi_o.tif'
        self.writeTiff(ndvi, ns, nl, 1, geog, proj, outfile)
        # outfile = wdir+r'heihe_result\heihe_'+dataTime+'_red_o.tif'
        # self.writeTiff(reflnewred, ns, nl, 1, geog, proj, outfile)
        # outfile = wdir+r'heihe_result\heihe_'+dataTime+'_nir_o.tif'
        # self.writeTiff(reflnewnir, ns, nl, 1, geog, proj, outfile)
    ...
    def setEmis_nadir(self, wdir, dataTime):
        ...
        infile = wdir + r'auxiliary\heihe_classif.tif'
        ...
        infile = wdir + 'heihe_result\heihe_' + dataTime + r'_ndvi_n.tif'
        ...
        emis1 = emis1_s * (1 - fvc) + emis1_v * fvc
        emis2 = emis2_s * (1 - fvc) + emis2_v * fvc
        outfile = wdir + r'heihe_result\heihe_' + dataTime + '_emis1_n.tif'
        self.writeTiff(emis1, ns, nl, 1, geog, proj, outfile)
        outfile = wdir + r'heihe_Result\heihe_' + dataTime + '_emis2_n.tif'
        self.writeTiff(emis2, ns, nl, 1, geog, proj, outfile)

        return 0

    def setEmis_obliq(self, wdir, dataTime):
        ...
        infile = wdir + 'heihe_result\heihe_' + dataTime + r'_ndvi_o.tif'
        dataset = gdal.Open(infile, gdal.GA_ReadOnly)
        data = dataset.GetRasterBand(1)
        ndvi = data.ReadAsArray()
        fvc = (ndvi - self.ndvi_min) / (self.ndvi_max - self.ndvi_min)
        ...
        emis1 = emis1_s * (1 - fvc) + emis1_v * fvc
        emis2 = emis2_s * (1 - fvc) + emis2_v * fvc
        outfile = wdir + r'heihe_result\heihe_' + dataTime + '_emis1_o.tif'
        self.writeTiff(emis1, ns, nl, 1, geog, proj, outfile)
        outfile = wdir + r'heihe_result\heihe_' + dataTime + '_emis2_o.tif'
        self.writeTiff(emis2, ns, nl, 1, geog, proj, outfile)

        return 0

    def inversionLST_nadir(self, wdir, dataTime):
        ...
        # coeffs = self.getDatafromTxt(infile,0,15)
        # coeffs = coeffs[:,2:10]
        infile = wdir + r'heihe\heihe_' + dataTime + '_bt8_n.tif'
        [BT8, ns, nl, nb, geog, proj] = self.getTiffData(infile)
        infile = wdir + r'heihe\heihe_' + dataTime + '_bt9_n.tif'
        [BT9, temp1, temp2, temp3, temp4, temp5] = self.getTiffData(infile)
        infile = wdir + r'heihe\heihe_' + dataTime + '_tcw_n.tif'
        [tcw, temp1, temp2, temp3, temp4, temp5] = self.getTiffData(infile)
        infile = wdir + r'heihe\heihe_' + dataTime + '_vza_n.tif'
        [vza, temp1, temp2, temp3, temp4, temp5] = self.getTiffData(infile)
        infile = wdir + r'heihe_result\heihe_' + dataTime + '_emis1_n.tif'
        [emis8, temp1, temp2, temp3, temp4, temp5] = self.getTiffData(infile)
        infile = wdir + r'heihe_result\heihe_' + dataTime + '_emis2_n.tif'
        ...
        ind = LST < 275
        LST[ind] = 0
        outfile = wdir + r'heihe_result\heihe_' + dataTime + '_LST_n.tif'
        self.writeTiff(LST, ns, nl, 1, geog, proj, outfile)

        return 0

    def inversionLST_obliq(self, wdir, dataTime):

        infile = wdir + r'\auxiliary\SLSTR_SW_Coeff_Day.txt'
        coeffs = self.getDatafromTxt(infile, 0, 15)
        coeffs = coeffs[:, 2:10]

        infile = wdir + r'heihe\heihe_' + dataTime + '_bt8_o.tif'
        [BT8, ns, nl, nb, geog, proj] = self.getTiffData(infile)
        infile = wdir + r'heihe\heihe_' + dataTime + '_bt9_o.tif'
        [BT9, temp1, temp2, temp3, temp4, temp5] = self.getTiffData(infile)
        infile = wdir + r'heihe\heihe_' + dataTime + '_tcw_n.tif'
        [tcw, temp1, temp2, temp3, temp4, temp5] = self.getTiffData(infile)
        infile = wdir + r'heihe\heihe_' + dataTime + '_vza_o.tif'
        [vza, temp1, temp2, temp3, temp4, temp5] = self.getTiffData(infile)
        infile = wdir + r'heihe_result\heihe_' + dataTime + '_emis1_o.tif'
        [emis8, temp1, temp2, temp3, temp4, temp5] = self.getTiffData(infile)
        infile = wdir + r'heihe_result\heihe_' + dataTime + '_emis2_o.tif'
        [emis9, temp1, temp2, temp3, temp4, temp5] = self.getTiffData(infile)
        ...
        ind = LST > 500
        LST[ind] = 0
        outfile = wdir + r'heihe_result\heihe_' + dataTime + '_LST_o.tif'
        self.writeTiff(LST, ns, nl, 1, geog, proj, outfile)

        return 0

    def inversionLST_nadir_night(self, wdir, dataTime,dataTimeold):
        ...
        [vza, temp1, temp2, temp3, temp4, temp5] = self.getTiffData(infile)
        infile = wdir + r'heihe_result\heihe_' + dataTimeold + '_emis1_n.tif'
        if os.path.exists(infile) == 0: return 0
        [emis8, temp1, temp2, temp3, temp4, temp5] = self.getTiffData(infile)
        infile = wdir + r'heihe_result\heihe_' + dataTimeold + '_emis2_n.tif'
        [emis9, temp1, temp2, temp3, temp4, temp5] = self.getTiffData(infile)
    ...
    def inversionLST_obliq_night(self, wdir, dataTime,dataTimeold):
        ...
        [vza, temp1, temp2, temp3, temp4, temp5] = self.getTiffData(infile)
        infile = wdir + r'heihe_result\heihe_' + dataTimeold + '_emis1_o.tif'
        if os.path.exists(infile) == 0:return 0
        [emis8, temp1, temp2, temp3, temp4, temp5] = self.getTiffData(infile)
        infile = wdir + r'heihe_result\heihe_' + dataTimeold + '_emis2_o.tif'
        [emis9, temp1, temp2, temp3, temp4, temp5] = self.getTiffData(infile)
    ...
    #################################
    def inversionLSCT(self, wdir, dataTime, off=2):
        ...
        infile = wdir + 'heihe_result\heihe_' + dataTime + r'_lst_n_new.tif'
        [lst_n, temp1, temp2, temp3, temp4, temp5] = self.getTiffData(infile)
        infile = wdir + 'heihe_result\heihe_' + dataTime + r'_lst_o_new.tif'
        ...
        for k1 in range(off, nl - off):
            ...
            for k2 in range(off, ns - off):
                ...
                # w = np.hstack((w1[:,ind]*fd[ind],w2[:,ind]*fd[ind]))
                # b = np.hstack((b1[ind]*fd[ind],b2[ind]*fd[ind]))
                # w = np.transpose(w)
                # coeffprior = np.asarray(lstsq(w, b))[0]
                ...
                Cdd = np.matrix(np.diag(np.ones(2)) * Cdvaluep)
                bb = sorted(b)
                halfpoint = np.int(2 / 2)
                ...
                ctprior = pre * (np.transpose(w) * np.linalg.inv(Cdd) * b + np.linalg.inv(Cmm) * coeffprior)
        ...
        outfile = wdir + r'heihe_result\heihe_' + dataTime + '_LSCT_'+str(off)+'_comb.tif'
    ...
    #################################
    def inversionLSCT_npl_agl(self, wdir, dataTime, off=2,off_ = 0):
        ...
        infile = wdir + 'heihe_result\heihe_' + dataTime + r'_ndvi_n_new.tif'
        dataset = gdal.Open(infile, gdal.GA_ReadOnly)
        data = dataset.GetRasterBand(1)
        ndvi = data.ReadAsArray()
        fvc_n = (ndvi - self.ndvi_min) / (self.ndvi_max - self.ndvi_min)
        fvc_n[fvc_n > 1] = 1
        fvc_n[fvc_n < 0] = 0
        infile = wdir + 'heihe_result\heihe_' + dataTime + r'_ndvi_o_new.tif'
        dataset = gdal.Open(infile, gdal.GA_ReadOnly)
        data = dataset.GetRasterBand(1)
        ndvi = data.ReadAsArray()
        fvc_o = (ndvi - self.ndvi_min) / (self.ndvi_max - self.ndvi_min)
        ...
        outfile = wdir + r'heihe_result\heihe_' + dataTime + '_LSCT_'+str(off)+'_new.tif'
        self.writeTiff(result, ns, nl, 2, geog, proj, outfile)
    ...
    #### the zhan multi-pixel method and multi-angle method
    #################################
    def inversionLSCT_npl_agl_point(self, wdir, dataTime, lat_,lon_,off=2, off_=0):
        ...
        infile = wdir + 'heihe_result\heihe_' + dataTime + r'_lst_n.tif'
        [lst_n, temp1, temp2, temp3, temp4, temp5] = self.getTiffData(infile)
        infile = wdir + 'heihe_result\heihe_' + dataTime + r'_lst_o.tif'
        [lst_o, temp1, temp2, temp3, temp4, temp5] = self.getTiffData(infile)
    ...
    #### the zhan multi-pixel method and multi-angle method
    #################################
    def inversionLSCT_npl_agl_point_sgl(self, wdir, dataTime, lat_,lon_,off=2, off_=0):
        ...
        outfile = wdir + r'heihe_result\heihe_' + dataTime + '_LSCT_' + str(off) + '_sgl.tif'
        self.writeTiff(result, ns, nl, 2, geog, proj, outfile)
    ...
    #### the zhan multi-pixel method and multi-angle method
    #################################
    def inversionLSCT_npl_agl_test(self, wdir, dataTime, off=2, off_=0):
        ...
        outfile = wdir + r'heihe_result\heihe_' + dataTime + '_LSCT_' + str(off) + '_test.tif'
        self.writeTiff(result, ns, nl, 2, geog, proj, outfile)
    ...
    #### the zhan multi-pixel method and multi-angle method
    #################################
    def inversionLSCT_npl_agl_point_test(self, wdir, dataTime, lat_,lon_, off=2, off_=0):
    ...
    #### the zhan multi-pixel method and multi-angle method
    #################################
    def inversionLSCT_npl_agl_raw(self, wdir, dataTime, off=2, off_=0):
        ...
        outfile = wdir + r'heihe_result\heihe_' + dataTime + '_LSCT_' + str(off) + '_raw.tif'
        self.writeTiff(result, ns, nl, 2, geog, proj, outfile)
    ...
    #### the zhan multi-pixel method and multi-angle method
    #################################
    def inversionLSCT_npl_agl_point_raw(self, wdir, dataTime, lat_,lon_, off=2, off_=0):
    ...
    def inversionLSCT_npl_agl_point_pur(self, wdir, dataTime, lat_,lon_, off=2, off_=0):
        ...
        outfile = wdir + r'heihe_result\heihe_' + dataTime + '_LSCT_' + str(off) + '_pur.tif'
        self.writeTiff(result, ns, nl, 2, geog, proj, outfile)

        return 0


    def inversionLSCT_npl_point(self, wdir, dataTime, lat_, lon_, off=2, off_=0):
        ...
        outfile = wdir + r'heihe_result\heihe_' + dataTime + '_LSCT_' + str(off) + '_npl.tif'
        self.writeTiff(result, ns, nl, 2, geog, proj, outfile)

        return 0

    def inversionLSCT_raw(self, wdir, dataTime, off_=0):
        ...
        outfile = wdir + r'heihe_result\heihe_' + dataTime + '_LSCT_'+str(off_)+'_raw.tif'
        self.writeTiff(result, ns, nl, 2, geog, proj, outfile)

        return 0

    def inversionLSCT_agl(self, wdir, dataTime, off=2,off_=0):
        ...
        outfile = wdir + r'heihe_result\heihe_' + dataTime + '_LSCT_'+str(off)+'_agl.tif'
        self.writeTiff(result, ns, nl, 2, geog, proj, outfile)

        return 0

    def inversionLSCT_agl_point(self, wdir, dataTime,lat_,lon_, off=0,off_=0):
    ...
    def inversionLSCT_backup(self, wdir, dataTime, off=2):
        ...
        outfile = wdir + r'heihe_result\heihe_' + dataTime + '_LSCT.tif'
        self.writeTiff(result, ns, nl, 1, geog, proj, outfile)

        return 0
    
    def inversionLSCT_2(self, wdir, dataTime, off=2):
        ...
        outfile = wdir + r'heihe_result\heihe_' + dataTime + '_LSCT_2.tif'
        self.writeTiff(result, ns, nl, 1, geog, proj, outfile)

        return 0

    def inversionLSCT_npl(self, wdir, dataTime, off=2):
        ...
        outfile = wdir + r'heihe_result\heihe_' + dataTime + '_LSCT_npl.tif'
        self.writeTiff(result, ns, nl, 1, geog, proj, outfile)

        return 0

    def inversionLSCT_night(self, wdir, dataTime,dataTimeold, off=2):
        ...
        infile = wdir + 'heihe_result\heihe_' + dataTimeold + r'_ndvi_n.tif'
        dataset = gdal.Open(infile, gdal.GA_ReadOnly)
        data = dataset.GetRasterBand(1)
        ndvi = data.ReadAsArray()
        fvc_n = (ndvi - self.ndvi_min) / (self.ndvi_max - self.ndvi_min)
        fvc_n[fvc_n > 1] = 1
        fvc_n[fvc_n < 0] = 0
        infile = wdir + 'heihe_result\heihe_' + dataTimeold + r'_ndvi_o.tif'
        dataset = gdal.Open(infile, gdal.GA_ReadOnly)
        data = dataset.GetRasterBand(1)
        ndvi = data.ReadAsArray()
        fvc_o = (ndvi - self.ndvi_min) / (self.ndvi_max - self.ndvi_min)
