...
def coeff_invert(kdoy,off,l1,l2,s1,s2,outdir):
    ...
    if ifcoeffb == 1:
        ...
        lst_nadir = np.zeros([nb * nband, nll, nss])
        lst_obliq = np.zeros([nb * nband, nll, nss])
        vza_nadir = np.zeros([nb * nband,  nll, nss])
        sza_nadir = np.zeros([nb * nband,  nll, nss])
        psi_obliq = np.zeros([nb * nband,  nll, nss])
        psi_nadir = np.zeros([nb * nband,  nll, nss])
        vza_obliq = np.zeros([nb * nband, nll, nss])
        sza_obliq = np.zeros([nb * nband,  nll, nss])
        clouds = np.zeros([nb * nband,nll,nss])


        tas = np.zeros([nb * nband , nll, nss])
        ...
        k = np.zeros([nll,nss])
        if iffirst == 1:
            for kdayNight in range(2):
                dayNight = dayNights[kdayNight]
                for ksate in range(2):
                    indir1 = indirs1[ksate]
                    indir2 = indirs2[ksate]
                    symbol = symbols[ksate]
                    kstep = nb * ksate + kdayNight * 2 * nb
                    for krun in range(nb):
                        ...
                        infile_ta = indir2 + symbol + '_2019%03d' % (kday + 1) + '_'+dayNight+'_ta.tif'
                        ...
                        if ((os.path.exists(infile_nadir_lst) ==1) * (os.path.exists(infile_nadir_vaa)==1)):
                            ...
                            tas[krun+kstep,:] = ta[l1:l2,s1:s2]*1.0
        ...
        for kl in range(nll):
            for ks in range(nss):
              ...
              #  print(np.sum(ind))
                ...
                temp = fitting_LSFRLE_ta_full(lst1,lst2,vza1,vza2,sza1,sza2,psi1,psi2,ta)
                a[kl,ks] = temp[0]
                b[kl,ks] = temp[1]
                k[kl,ks] = temp[2]
        a[a> 1000] = 0
...
if __name__ == '__main__':
    ...
    s1 = 900
    s2 = 1100
    outdir = r'D:\data\an/Bcoeff_direct/'
